...
engine_data_processor_db = create_engine('postgresql+psycopg2://postgres:postgres@localhost:5433/data_processor_db')
...
def get_dataset(interval_start: date, interval_end: date):
    sql = text("""select tab1.package_id, tab1.quant/qrd.quant_resources :: float, tab1.quant, qrd.quant_resources
                from quant_resources_dataset qrd, (
                select ti.package_id, count(ti.package_id) as quant
                from temporal_index ti
                where get_similarity(:interval_start_, :interval_end_, ti.interval_start, ti.interval_end, 0.5, 0.5)>0
                group by package_id
                ) as tab1
                where tab1.package_id like qrd.id  """)
    index = engine_temporal_indexing_module\
        .execute(sql, {'interval_start_': interval_start, 'interval_end_': interval_end}).fetchall()
    resources = {}
    for i in index:
